...
export const UserAvatarSection: React.FC = () => {
  ...
  if (!user) return null;
  return <Card className="brutalist-card">
      <CardHeader>
      ...
      <CardContent className="space-y-4">
        <div className="flex items-center gap-4">
          ...
          <Button onClick={generateRandomAvatar} variant="outline" size="sm" className="brutalist-button">
            <RefreshCw size={16} />
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-5 gap-3">
            {avatarOptions.map(avatar => <button key={avatar.id} onClick={() => handleAvatarSelect(avatar.id)} className={`relative group ${selectedAvatar === avatar.id ? 'ring-2 ring-accent' : ''}`}>
                ...
                {selectedAvatar === avatar.id && <div className="absolute -top-1 -right-1 w-4 h-4 bg-accent border-2 border-background rounded-full" />}
              </button>)}
          </div>
        </div>
