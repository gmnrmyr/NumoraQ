-- Safe migration that only adds missing features without breaking existing ones

-- 1. Add missing columns if they don't exist
DO $$ 
BEGIN
    -- Add source tracking to user_premium_status if missing
    ...
    -- Add source tracking to user_points if missing
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
...
-- 2. Fix foreign key constraint for activated_code if needed
DO $$
BEGIN
    -- Drop and recreate activated_code constraint to handle NULL properly
    IF EXISTS (SELECT 1 FROM information_schema.table_constraints 
               WHERE constraint_name = 'user_premium_status_activated_code_fkey') THEN
        ALTER TABLE public.user_premium_status
        DROP CONSTRAINT user_premium_status_activated_code_fkey;
    END IF;
    
    -- Add it back with proper NULL handling
    IF EXISTS (SELECT 1 FROM information_schema.columns 
               WHERE table_name='user_premium_status' 
               AND column_name='activated_code') THEN
        ALTER TABLE public.user_premium_status
...
-- 3. Update premium_type constraint to include all valid types
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.check_constraints 
               WHERE constraint_name = 'user_premium_status_premium_type_check') THEN
        ALTER TABLE public.user_premium_status 
        DROP CONSTRAINT user_premium_status_premium_type_check;
...
-- 4. Fix user_points constraints to allow multiple manual entries
DO $$
BEGIN
    -- Drop problematic unique constraint if it exists
    IF EXISTS (SELECT 1 FROM information_schema.table_constraints 
               WHERE constraint_name = 'user_points_user_id_activity_type_activity_date_key') THEN
        ALTER TABLE public.user_points 
        DROP CONSTRAINT user_points_user_id_activity_type_activity_date_key;
    END IF;
    
    -- Create partial unique index only for daily_login and referral
...
-- 5. Create admin views for source tracking (replace if exists)
...
-- 6. Update existing records to have proper source tracking where missing
...
-- 7. Ensure trial users exist for users without premium status
